    ...
    guild.members.cache.forEach((member) => {
      ...
      if (
        ...
        !member.presence.activities.find(
          (presence) => presence.name === role.name.replace("-ingame", "")
        )
      ) {
        member.roles
          .remove(role)
          .then(
            () =>
              `stale role "${role.name}" removed from "${member.displayName}"`
          )
          .catch((reason) => {
            console.log(
              `failed to remove stale role "${role.name}" from "${member.name}"! reason: ${reason}`
            );
          });
      }
    });
...
client.on("presenceUpdate", (oldPresence, newPresence) => {
  ...
  if (
    ...
    !member.presence.activities.some((a) => !!a.applicationId)
  ) {
    // not playing a game
    return;
  }
  newPresence.activities
    ...
    .forEach(async (activity) => {
      ...
      member.roles
        .add(role)
        .then(() => `role "${role.name}" added to "${member.displayName}"`)
        .catch((reason) => {
          console.log(
            `failed to add role "${role.name}" to "${member.name}"! reason: ${reason}`
          );
        });
    });
  newPresence.activities
    ...
    .forEach(async (activity) => {
      ...
      if (hasRole(member, inGameRoleName)) {
        if (process.env.NODE_ENV !== "production") {
          console.log(
            `"${member.displayName}" already has role "${inGameRoleName}"`
          );
        }
        return;
      }
      ...
      member.roles
        .add(inGameRole)
        .then(
          () => `role "${inGameRole.name}" added to "${member.displayName}"`
        )
        .catch((reason) => {
          console.log(
            `failed to add role "${inGameRole.name}" to "${member.name}"! reason: ${reason}`
          );
        });
    });
  oldPresence.activities
    ...
    .forEach(async (activity) => {
      ...
      if (!hasRole(member, inGameRoleName)) {
        if (process.env.NODE_ENV !== "production") {
          console.log(
            `"${member.displayName}" doesn't have role "${inGameRoleName}"`
          );
        }
        return;
      }
      ...
      member.roles
        .remove(inGameRole)
        .then(
          () => `role "${inGameRole.name}" removed from "${member.displayName}"`
        )
        .catch((reason) => {
          console.log(
            `failed to remove role "${inGameRole.name}" from "${member.name}"! reason: ${reason}`
          );
        });
    });
});
